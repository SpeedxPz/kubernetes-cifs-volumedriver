...
func TestMountCmdNewline(t *testing.T) {
...
func TestMountCmdReturn(t *testing.T) {
...
func TestPassEnvOption(t *testing.T) {
	...
	if !found {
		t.Error("TestPassEnvOption env variable expected PASSWD=pass123")
	}

	if len(mountCmd.Args) != len(expected) {
		t.Errorf("TestPassEnvOption len: expected %d, actual %d", len(mountCmd.Args), len(expected))
	}

	for idx := range expected {
		if mountCmd.Args[idx] != expected[idx] {
			t.Errorf("TestPassEnvOption[%d]: expected %s, actual %s", idx, expected[idx], mountCmd.Args[idx])
		}
	}
}
